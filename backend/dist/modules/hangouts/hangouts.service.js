...
export const listPublicRooms = async (status) => {
    return prisma.room.findMany({
        where: { visibility: 'PUBLIC', status },
        orderBy: { createdAt: 'desc' },
    });
};
export const createRoom = async (hostId, title, visibility) => {
    const channelName = `room_${crypto.randomUUID()}`;
    let joinToken = null;
    let joinTokenHash = null;
...
export const joinRoom = async (roomId, userId, joinToken) => {
